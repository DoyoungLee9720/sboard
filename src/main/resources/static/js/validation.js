...
window.onload = function(){
	...
	btnCheckUid.onclick = function(){
		...
		// 중복체크
		fetch(`/sboard/check?type=uid&value=` + uid)
			.then(resp => resp.json())
			.then(data => {
				console.log(data);
	...
	registerForm.nick.addEventListener('focusout', function(){
		...
		fetch(`/sboard/check?type=nick&value=` + nick)
	...
	btnSendEmail.onclick = async function(){
		...
		try{
			...
			const response = await fetch('/sboard/check?type=email&value='+email);
	...
	registerForm.hp.addEventListener('focusout', async function(){
		...
		try{
			const response = await fetch('/sboard/check?type=hp&value='+hp);
			const data = await response.json();
			console.log(data);
